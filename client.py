...
# Boilerplate Code
# 3. to receive message from server and accordingly filling listbox/ textarea 
def receiveMessage():
    global SERVER
    global BUFFER_SIZE
    ...
    while True:
        ...
        try:
            #Filling ListBox with clients who join
            ...
            #Else, filling textarea with messages from server
            else:
                textarea.insert(END,"\n"+chunk.decode('ascii'))
                textarea.see("end")
...
# to ask server to show list of clients in listbox, we send "show list" message to server
# 2. called when refresh button is pressed, sends message 'show list' to server
#def showClientsList():
...
# 1. called on pressing connect button, we send our name to server
def connectToServer():
    global SERVER
    global name
    global sending_file
